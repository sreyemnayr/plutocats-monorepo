...
import { PlutocatsToken, PlutocatsReserve, ReserveGovernor, PlutocatsDescriptor, PlutocatsReserveV2, ReserveGovernorV2, MockWithdrawable } from '../../typechain';
...
describe("ReserveGovernor and Bootstrap process with upgrade to v2", function () {
    ...
    beforeEach(async function () {
        ...
        const contractsV2: Record<UpgradedContractName, DeployedContract> = await run('deploy-v2-upgrades', {
            autodeploy: true,
        ...
        // quit the first token
        await plutocatsToken.connect(wallet).approve(plutocatsReserve.address, 0);
        await plutocatsReserve.connect(wallet).quit([0]);
